 ...
 * @param id//主键字段
 */
function toShenhe(title,url,gridId,width,height) {
	width = '1050px';
	height = '750px';
	//url="dmwhproject/{id}/shenheProject";
    ...
    if (!rowsData || rowsData.length==0) {
	    top.layer.alert('请至少选择一条数据1111!', {icon: 0, title:'警告'});
		return; 
	}
	...
	$.get("dmwhproject/"+id+"/checkVerify", function(data, status) {
		//alert(data);
		if(data=="0"){
			top.layer.alert('所选项目已经审核过了!', {icon: 0, title:'警告'});
			
		}else{
			url="/CrowdFounding/shenhe.jhtml?id="+id;
	...
	top.layer.open({
	    ...
	    btn: ['关闭'],
		 cancel: function(index){ 
			// window.location.href="/CrowdFoundingMS//admin/crowdfunding/dmwhproject";
		 }
	}); 	
